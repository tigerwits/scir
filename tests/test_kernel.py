...
from scir import (Term, Var, Node, ParseError, parse, parse_document, parse_pattern,
                  format_document, validate, digest, match, instantiate, query,
                  walk, at, replace_at, diff)
...
class SyntaxTests(unittest.TestCase):
    def test_six_original_examples(self):
        text = (ROOT/'examples/six_sentences.scir').read_text()
        doc = parse_document(text)
        self.assertEqual(len(doc), 6)
        self.assertEqual(format_document(doc), text)
