...
from scir import (
    Node, Term, Var, at, diff, match,
    parse_document, parse_pattern, query, replace_at, walk,
)
...
class ResourceTests(unittest.TestCase):
    ...
    def test_macro_definition_checked_before_substitution(self):
        body = Node('A')
        for _ in range(600):
            body = Node('f', (body,))
        with self.assertRaisesRegex(ValueError, 'resource limit'):
            expand((Term('m'),), {'m': Definition((), body)}, max_steps=10)

    def test_macro_budget_also_bounds_unused_definitions(self):
        body = Node('wide', (Node('A'),) * 20)
        with self.assertRaisesRegex(ValueError, 'resource limit'):
            expand((), {'m': Definition((), body)}, max_steps=5)

    def test_macro_unused_bad_arity_rejected(self):
        definitions = {'f': Definition(('x',), Node('A')), 'g': Definition((), Node('f'))}
        with self.assertRaisesRegex(ValueError, 'arity mismatch'):
            expand((), definitions)

    def test_macro_dependency_order_does_not_change_output(self):
        definitions = {
            'f': Definition(('x',), parse_pattern('pair(g(?x), g(?x))')),
            'g': Definition(('x',), parse_pattern('wrapped(?x)')),
        }
        document = parse_document('f(A); f(B)')
        expected = parse_document('pair(wrapped(A), wrapped(A)); pair(wrapped(B), wrapped(B))')
        self.assertEqual(expand(document, definitions), expected)
        self.assertEqual(expand(document, dict(reversed(list(definitions.items())))), expected)
        self.assertEqual(expand(expected, definitions), expected)

    def test_expanded_arguments_count_as_occurrences(self):
        definitions = {'dup': Definition(('x',), parse_pattern('pair(?x, ?x)'))}
        self.assertEqual(expand(parse_document('dup(A)'), definitions), parse_document('pair(A,A)'))
        with self.assertRaisesRegex(ValueError, 'resource limit'):
            expand(parse_document('dup(dup(dup(dup(A))))'), definitions, max_steps=40)

    def test_shared_pattern_does_not_explode_variable_collection(self):
        body = Var('x')
        for _ in range(50):
            body = Node('pair', (body, body))
        self.assertEqual(variables(body), {'x'})
        definition = Definition(('x',), body)
        with self.assertRaisesRegex(ValueError, 'resource limit'):
            expand((), {'dup': definition}, max_steps=20)
...
class CliBoundaryTests(unittest.TestCase):
    ...
    def test_duplicate_transport_keys_rejected(self):
        source = '{"version":"wrong","version":"scir-relations/0.2","nodes":[],"args":[],"roots":[]}'
